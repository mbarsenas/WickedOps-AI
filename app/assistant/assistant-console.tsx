...
function intelligenceText(result: any) {
  ...
  if (Array.isArray(result)) {
    return result.map((item) => `${item.label ?? item.provider}: ${item.text ?? item.error ?? "No response"}`).join("\n\n");
  }
...
export default function AssistantConsole({ displayName }: { displayName: string }) {
  ...
  async function toggleVoice() {
    ...
    try {
      ...
      pc.ontrack = (event) => {
        ...
        startPlayback(stream).catch(() => setError("Your browser blocked Sable's speaker. Tap the microphone once more."));
      };
      ...
      dc.addEventListener("open", () => {
        ...
        dc.send(JSON.stringify({ type: "response.create", response: { instructions: "Greet Mark warmly in one short sentence and ask what he would like to do." } }));
      });
      dc.addEventListener("message", (event) => {
        ...
        if ((message.type === "response.output_audio_transcript.delta" || message.type === "response.audio_transcript.delta") && message.delta) {
          setReply((current) => current === "Thinking…" || current === "I’m listening." ? message.delta : current + message.delta);
        }
        if (message.type === "response.done") setMode("listening");
        if (message.type === "error") { setError(message.error?.message || "Sable reported a voice error."); setMode("ready"); }
      });
  ...
  async function sendText(event: FormEvent) {
    ...
    if (intelligenceMode === "sable") {
      if (channelRef.current?.readyState !== "open") {
        setError("Start the voice session for Sable mode, or select GPT, Claude, Grok, Ask All, or Collaborate.");
        setMode("ready");
        return;
      }
      channelRef.current.send(JSON.stringify({ type: "conversation.item.create", item: { type: "message", role: "user", content: [{ type: "input_text", text }] } }));
    ...
    try {
      const response = await fetch("/api/intelligence", {
        ...
        body: JSON.stringify(intelligenceRequest(intelligenceMode, text)),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result?.error || "Multi-model request failed.");
      setReply(intelligenceText(result));
      setMode(connected ? "listening" : "ready");
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Multi-model request failed.");
      setReply("The selected model could not complete the request.");
      setMode(connected ? "listening" : "ready");
    }
  }
      ...
      <section className="assistant-room">
        ...
        <div className="assistant-orb-space">
          <div className={`account-orb-shell ${mode}`} ref={orbRef}>
            <div className="account-orbit orbit-a"><i /></div><div className="account-orbit orbit-b"><i /></div>
            <div className="account-orb"><div className="account-core" /><div className="account-scan" /><span className="wave-ring ring-one" /><span className="wave-ring ring-two" /></div>
          </div>
        </div>

        <div className="assistant-identity"><span className={`presence ${connected ? "online" : ""}`} /><strong>Sable</strong><small>{mode}</small></div>
        <p className="assistant-words" aria-live="polite">{reply}</p>

        <div className="intelligence-modes" aria-label="AI mode">
          {intelligenceModes.map((item) => (
            <button key={item.key} type="button" className={intelligenceMode === item.key ? "selected" : ""} onClick={() => setIntelligenceMode(item.key)}>{item.label}</button>
          ))}
        </div>

        <form className="assistant-command" onSubmit={sendText}>
          <button type="button" className={connected ? "active" : ""} onClick={toggleVoice} aria-label={connected ? "End voice session" : "Start voice session"}>{connected ? <Pause size={20} /> : <Mic size={20} />}</button>
          <input value={command} onChange={(event) => setCommand(event.target.value)} placeholder={`Ask ${intelligenceModes.find((item) => item.key === intelligenceMode)?.label ?? "Sable"} anything…`} aria-label="Message Sable" />
          <button type="submit" aria-label="Send message"><ArrowRight size={19} /></button>
        </form>
        <div className={`assistant-note ${error ? "error" : ""}`}>{error || (connected ? "Live voice connected · interrupt Sable naturally" : intelligenceMode === "sable" ? "Tap the microphone to begin" : `${intelligenceModes.find((item) => item.key === intelligenceMode)?.label} text mode ready`)}</div>
