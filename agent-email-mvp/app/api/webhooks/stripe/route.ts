...
export async function POST(req:Request){
 ...
 try{
  if(['customer.subscription.created','customer.subscription.updated','customer.subscription.deleted'].includes(event.type)){
   ...
   const sql=db();await sql`UPDATE organizations SET stripe_subscription_id=${subscription.id},subscription_status=${active?subscription.status:'free'},monthly_limit=${limit},billing_event_at=${event.created} WHERE stripe_customer_id=${customer} AND id=${subscription.metadata.organization_id||'00000000-0000-0000-0000-000000000000'} AND billing_event_at<=${event.created}`;
  }
