import {transportName,localLab} from '../../../lib/transport/active';
...
export async function GET(req:Request,{params}:{params:Promise<{path:string[]}>}){
 ...
 return Response.json({alerts:await syncAlerts(org),agents,identities,policies,conversations,approvals,actions,audit:auditRows,jobs,apiKeys,webhookEndpoints,emailApiEvents,workspace:workspaceRows[0],ai_usage:await aiUsage(org),usage:usageRows[0]||{accepted:0,reserved:0},stats:stats[0],user:user.email,config:{transport:transportName,local_lab:localLab,billing:billingConfigured(),database:true,resend:!!process.env.RESEND_API_KEY,ai:!!process.env.OPENAI_API_KEY,webhook:!!process.env.RESEND_WEBHOOK_SECRET,webhook_url:(process.env.APP_BASE_URL||'')+'/api/webhooks/resend',api_base:(process.env.APP_BASE_URL||'')+'/api/v1',model:process.env.OPENAI_MODEL||'gpt-5-mini'}},{headers:{'Cache-Control':'private, no-store'}});
 }
