...
export function deliveryInfo(workspace:string){
 ...
 return {name:pilot?'SenderPermit · AWS':'Resend',sending_enabled:!pilot||(ready&&process.env.SENDERPERMIT_PILOT_SENDING==='enabled'),pilot};
}
export function pilotTransport(workspace:string):OutboundTransport|null{
 if(workspace!==PILOT_WORKSPACE)return null;
 if(!deliveryInfo(workspace).sending_enabled)throw new HttpError(503,'Sending is paused for this test workspace while AWS delivery is being activated. No email was queued.');
