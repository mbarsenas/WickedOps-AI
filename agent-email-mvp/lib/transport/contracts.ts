...
export class SenderPermitTransport implements OutboundTransport {
 ...
 async submit(workspace:string,key:string,message:OutboundMessage):Promise<Submission>{
  const r=await this.request(new URL('/v1/submissions',this.origin),{method:'POST',redirect:'manual',headers:{authorization:'Bearer '+this.token,'content-type':'application/json','idempotency-key':key},body:JSON.stringify({workspace,message}),signal:AbortSignal.timeout(15000)});
