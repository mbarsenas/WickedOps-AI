...
export async function captureInbound(providerId:string,email:{from:string;to:string[];subject:string;text:string|null},scopedWorkspace?:string){
 const sql=db();const domains=email.to.map(to=>to.toLowerCase().split('@').pop()!);
 const owners=scopedWorkspace?domains.map(name=>({organization_id:scopedWorkspace,name})):await sql`SELECT organization_id,name FROM sending_domains WHERE name=ANY(${domains}) AND receiving='enabled'`;
