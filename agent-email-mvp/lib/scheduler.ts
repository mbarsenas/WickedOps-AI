import {syncAWSInbound} from './inbound-sync';
...
export async function runWebhookSchedule(){
 ...
 try{
  ...
  const orgs=await sql`SELECT id FROM organizations`;for(const org of orgs){await syncAWSInbound(org.id);await syncAlerts(org.id);}
