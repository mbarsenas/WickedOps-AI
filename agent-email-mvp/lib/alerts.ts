...
export async function syncAlerts(org:string){
 ...
 for(const [source,value] of Object.entries(snapshot.items||{})){const item=value as any;if(item.attention&&!rows.some(r=>r.source_key===source))rows.push({source_key:source,category:'email',title:'AWS delivery needs attention',detail:item.detail});}
 if(snapshot.unavailable)rows.push({source_key:'aws/status',category:'email',title:'Mail status checks are unavailable',detail:'Latest AWS delivery status could not be retrieved. Existing results have been kept. Refresh to retry; do not resend solely because this check failed.'});
