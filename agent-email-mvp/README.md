...
September 3 verification: billing-handler and local-network webhook tests passed. GitHub workflow is active and its manual invocation passed; scheduled failure/recovery has not been observed. The sandbox runner passed against AgentMail sandbox: paid invoice and active subscription granted 10,000 recipients; cancellation restored 100; declined initial payment retained 100. Checkout session creation passed. Signed events were delivered locally to the real handler, which retrieved actual Stripe subscriptions; hosted Checkout completion and external Stripe webhook transport remain unverified. Test subscriptions/customers were removed and test catalog entries archived. Production billing and tax settings are unchanged.

External Stripe verification completed September 3: the user completed sandbox hosted Checkout; Stripe confirmed the invoice paid and subscription active. A temporary HTTPS tunnel forwarded real Stripe-signed subscription.updated and subscription.deleted requests to the actual app handler connected only to the isolated Neon branch. Both returned HTTP 200; the workspace allowance changed from 100 to 10,000 and back to 100 on cancellation. Temporary endpoint removed and test subscription canceled. Production configuration was unchanged. The remaining deployment-specific distinction is that this transport check used the isolated local receiver, not the live Sites billing endpoint.
