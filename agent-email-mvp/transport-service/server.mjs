...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
 ...
 const q=new Queue(process.env.TRANSPORT_DB||'transport.sqlite');const server=service(q,accounts);server.requestTimeout=20000;server.headersTimeout=10000;server.listen(Number(process.env.PORT||4380),'127.0.0.1',()=>console.log('Transport listening on loopback'));process.on('SIGTERM',()=>server.close(()=>{q.close();process.exit(0);}));
}
